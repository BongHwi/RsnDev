...
bong-hwi.lim@cern.ch - last modified on 26/02/2019
****************************************************************************/

AliRsnMiniAnalysisTask *AddSuperTaskMC(
              Int_t collSystem        = AliPIDResponse::kPBPB,
              UInt_t triggerMask      = AliVEvent::kINT7,
              const char *QAParticles = "All",
...
{  
  ...
  // QA Particle Selection
  TString QAPart(QAParticles);
  ...
  // selections for Xi
  Float_t XiPIDcut=3.; Float_t V0dDCA=1.6; Float_t XidDCA=1.6; Float_t XiMinDCA=0.07; Float_t Xi_massTol=0.007;
  Float_t Xi_massTolVeto=0.007; Float_t V0CosPoinAn=0.97; Float_t XiCosPoinAn=0.97;
  ...
  const Int_t Np = 9;
  TString partname [Np] = {"Rho","Kstar","aKstar", "Phi", "Lstar", "aLstar", "f0", "f2", "Xi1530"};
  Int_t    pdgCode [Np] = {113, 313, -313, 333, 3124, -3124, 9010221, 225, 3324};
  Float_t mass     [Np] = {0.770, 0.89495, 0.89445, 1.019461, 1.51954, 1.51954, 0.990, 1.270, 1.53178};
  Float_t masslow  [Np] = {0.5, 0.7, 0.7, 0.9, 1.3, 1.3, 0.8, 0.8, 1.2};
  Float_t massup   [Np] = {1.0, 1.0, 1.0, 1.1, 1.7, 1.7, 1.2, 1.6, 1.8};
  Char_t  charge1  [Np] = {'+', '+','-','+','+','-','+', '+', '-'};
  Char_t  charge2  [Np] = {'-', '-','+','-','-','+','-','-', '+'};
  ...
  RSNPID d1[Np] = {AliRsnDaughter::kPion, AliRsnDaughter::kKaon, AliRsnDaughter::kKaon,
                   ...
                   AliRsnDaughter::kPion, AliRsnDaughter::kPion, AliRsnDaughter::kXi};
  ...
  for (Int_t i = 0; i<Np; i++){
    ...
    if(!QAPart.Contains("ALL") && !QAPart.Contains(partname[i])) continue;
    
    Int_t icut1, icut2;
    if (d1[i]==AliRsnDaughter::kProton) icut1 = icutPro;
    else if (d1[i]==AliRsnDaughter::kKaon) icut1 = icutKa;
    else if (d1[i]==AliRsnDaughter::kXi) icut1 = icutXi;
    else icut1 = icutPi;
    
    if (d2[i]==AliRsnDaughter::kProton) icut2 = icutPro;
    else if (d2[i]==AliRsnDaughter::kKaon) icut2 = icutKa;
    else if (d2[i]==AliRsnDaughter::kXi) icut2 = icutXi;
    else icut2 = icutPi;
